...
var rooms = new Rooms();
var Clients = require('../lib/clientsManager');
...
//  WSS_HOST_PORT_PAIRS: ['apprtc-ws.webrtc.org:443', 'apprtc-ws-2.webrtc.org:443'],
  WSS_HOST_PORT_PAIRS: [ADDRESS+':8000'],
...
function infoRecord(clientId, roomId) {
  MongoClient.connect(url, function(err, db) {
    // Get the collection
    var col = db.collection('userRecord');
    col.insertOne({
      clientId: clientId,
      roomId: roomId
    }).then(function() {
...
function addClientToRoom(req, roomId, clientId, isLoopback, callback) {
  ...
  rooms.createIfNotExist(key, function(error, room) {
    ...
    clients.add(room.hasClient(clientId), clientId, roomId);
  });
}
